...
@pytest.mark.parametrize(
    "text,expected_chars_count,expected_spaces_count,expected_chars_without_spaces_count", text_to_return_value_mapping
)
def test_given_text_when_parsed_then_return_empty_or_valid_list_or_counts(
    text: str, expected_chars_count: int, expected_spaces_count: int, expected_chars_without_spaces_count: int
):
    # given, when
...
@pytest.mark.parametrize(
    "text,expected_repeated_spaces,expected_repeated_spaces_count", text_to_repeated_counts_mapping
)
def test_given_text_when_parsed_then_return_empty_or_valid_list_or_repeated_counts(
    text: str, expected_repeated_spaces: list, expected_repeated_spaces_count: int
):
    # given, when
    actual_result = gather_repeated_spaces(text)

    # then
    assert (expected_repeated_spaces is actual_result) or (expected_repeated_spaces == actual_result), (
    ...
    # then
    assert (expected_repeated_spaces_count is actual_count) or (expected_repeated_spaces_count == actual_count), (
