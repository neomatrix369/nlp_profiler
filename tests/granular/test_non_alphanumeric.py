...
from nlp_profiler.granular_features.non_alphanumeric import gather_non_alpha_numeric, count_non_alpha_numeric  # noqa
...
def test_given_invalid_text_when_parsed_then_return_empty_list(text: str, expected_result: str):
    # given, when
...
def test_given_invalid_text_when_counted_then_return_NaN(text: str, expected_result: float):
    # given, when
    actual_result = count_non_alpha_numeric(text)

    # then
    assert expected_result is actual_result, f"Expected: {expected_result}, Actual: {actual_result}"
...
def test_given_a_text_with_non_alphanumeric_chars_when_parsed_then_return_non_alphanumeric_chars():
    ...
    # then
    assert expected_results == actual_results, "Didn't find the expected non-alphanumeric chars in the text with emoji"
...
def test_given_a_alphanumeric_text_when_counted_then_return_number_of_alphanumeric_chars():
    ...
    # then
    assert actual_results == 5, "Didn't find the expected number of non-alphanumeric " "chars in the text with a number"

    # given, when
    actual_results = count_non_alpha_numeric(text_with_emojis)

    # then
    assert actual_results == 7, "Didn't find the expected number of non-alphanumeric " "chars in the text with emojis"
