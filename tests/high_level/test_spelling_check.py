...
from nlp_profiler.high_level_features.spelling_quality_check import (
...
@pytest.mark.parametrize(
    "text," "expected_spelling_check_score," "expected_spelling_quality," "expected_spelling_check_summarised",
    text_to_return_value_mapping,
)
def test_given_a_text_when_spell_check_is_applied_then_spell_check_analysis_info_is_returned(
    ...
    if math.isnan(expected_spelling_check_score):
        ...
        assert math.isclose(
            expected_spelling_check_score, actual_score, rel_tol=1e-09, abs_tol=0.0
        ), "Spell check score didn't match for the text"

    # given, when
    actual_spelling_check = spelling_quality(actual_score)
    # then
    assert expected_spelling_quality == actual_spelling_check, "Spelling quality check didn't match for the text"
...
@pytest.mark.parametrize(
    "original_score,normalised_score,expected_spelling_quality_in_words", spelling_check_score_to_words_mapping
)
def test_given_spelling_check_score_when_converted_to_words_then_return_right_word(
    ...
    print(
        ...
        "normalised_score: ",
        normalised_score,
        "expected_spelling_quality_in_words:",
        expected_spelling_quality_in_words,
    )
