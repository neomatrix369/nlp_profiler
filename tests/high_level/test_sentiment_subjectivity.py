...
from nlp_profiler.high_level_features.sentiment_subjectivity import (
...
@pytest.mark.parametrize(
    "text," "expected_score," "expected_subjectivity," "expected_subjectivity_summarised", text_to_return_value_mapping
)
def test_given_a_text_when_sentiment_subjectivity_analysis_is_applied_then_subjective_analysis_info_is_returned(
    text: str, expected_score: float, expected_subjectivity: str, expected_subjectivity_summarised: str
):
    # given, when
    ...
    if math.isnan(expected_score):
        ...
        assert math.isclose(
            expected_score, actual_score, rel_tol=1e-09, abs_tol=0.0
        ), "Subjectivity/objectivity score didn't match for the text"
    # given, when
    actual_subjectivity = sentiment_subjectivity(actual_score)
    # then
    assert expected_subjectivity == actual_subjectivity, "Sentiment subjectivity didn't match for the text"
    # given,  when
    actual_subjectivity_summarised = sentiment_subjectivity_summarised(actual_subjectivity)
    # then
    assert (
        expected_subjectivity_summarised == actual_subjectivity_summarised
...
@pytest.mark.parametrize(
    "original_score,normalised_score,expected_subjectivity_in_words", sentiment_subjectivity_score_to_words_mapping
)
def test_given_sentiment_subjectivity_score_when_converted_to_words_then_return_right_word(
    ...
    print(
        ...
        "normalised_score: ",
        normalised_score,
        "expected_subjectivity_in_words:",
        expected_subjectivity_in_words,
    )
