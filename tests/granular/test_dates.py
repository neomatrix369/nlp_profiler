...
from nlp_profiler.granular_features.dates import gather_dates, count_dates  # noqa
...
def test_given_a_text_with_one_or_more_dates_when_counted_then_return_number_of_dates_found():
    ...
    # then
    assert actual_results == 1, "Didn't find the expected single date in the text"

    # given, when
    actual_results = count_dates(text_with_dates)

    # then
    assert actual_results == 2, "Didn't find the expected two dates in the text"
