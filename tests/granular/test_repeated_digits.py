...
from nlp_profiler.granular_features.numbers import gather_repeated_digits, count_repeated_digits  # noqa
...
def test_given_a_text_when_parsed_then_return_empty_or_a_valid_list(text: str, expected_result: str):
    ...
    # then
    assert expected_result == actual_result, "Didn't find the expected digits in the text"
...
def test_given_a_text_when_counted_then_return_NaN_or_count_of_digits(text: str, expected_result: float):
    # given, when
    actual_result = count_repeated_digits(text)

    # then
    assert expected_result is actual_result, "Didn't find the expected number of digits in the text"
    f"Expected: {expected_result}, Actual: {actual_result}"
