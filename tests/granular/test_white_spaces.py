...
from nlp_profiler.granular_features.chars_spaces_and_whitespaces \
    import count_chars, count_whitespaces, count_characters_excluding_whitespaces, \
    gather_repeated_whitespaces, count_repeated_whitespaces  # noqa
...
def test_given_text_when_parsed_then_return_empty_or_valid_list_or_counts(
        ...
        expected_whitespaces_count: int,
        expected_chars_without_whitespaces_count: int
):
    # given, when
...
@pytest.mark.parametrize("text,expected_repeated_whitespaces,expected_repeated_whitespaces_count",
                         text_to_repeated_counts_mapping)
def test_given_text_when_parsed_then_return_empty_or_valid_list_or_repeated_counts(
        text: str,
        expected_repeated_whitespaces: list,
        expected_repeated_whitespaces_count: int
):
    # given, when
    actual_result = gather_repeated_whitespaces(text)

    # then
    assert (expected_repeated_whitespaces is actual_result) or \
           (expected_repeated_whitespaces == actual_result), \
