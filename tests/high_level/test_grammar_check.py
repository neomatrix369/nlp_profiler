...
def test_given_a_correct_text_when_grammar_check_is_applied_then_no_grammar_issues_is_returned(
    ...
    assert actual_score is expected_score, (
        f"Grammar quality score should NOT " f"have been returned, expected {expected_score}"
    )
    ...
    assert actual_quality == expected_quality, (
        f"Grammar quality should NOT " f"have been returned, expected {expected_quality}"
    )
...
@pytest.mark.parametrize("score,expected_result", grammar_check_score_to_words_mapping)
