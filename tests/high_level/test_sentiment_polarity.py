...
from nlp_profiler.high_level_features.sentiment_polarity import (
...
@pytest.mark.parametrize(
    "text," "expected_polarity_score," "expected_polarity," "expected_polarity_summarised", text_to_return_value_mapping
)
def test_given_a_text_when_sentiment_analysis_is_applied_then_sentiment_analysis_info_is_returned(
    text: str, expected_polarity_score: float, expected_polarity: str, expected_polarity_summarised: str
):
    # given, when
    ...
    if math.isnan(expected_polarity_score):
        ...
        assert math.isclose(
            expected_polarity_score, actual_score, rel_tol=1e-09, abs_tol=0.0
        ), "Sentiment polarity score didn't match for the text"

    # given, when
    actual_polarity = sentiment_polarity(actual_score)
    # then
    assert expected_polarity == actual_polarity, "Sentiment polarity didn't match for the text"

    # given, when
    actual_summarised_polarity = sentiment_polarity_summarised(actual_polarity)
    # then
    assert (
        expected_polarity_summarised == actual_summarised_polarity
...
@pytest.mark.parametrize(
    "original_score,normalised_score,expected_sentiment_in_words", sentiment_polarity_score_to_words_mapping
)
