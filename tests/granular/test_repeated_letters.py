...
from nlp_profiler.granular_features.letters import gather_repeated_letters, count_repeated_letters  # noqa
...
def test_given_a_text_when_parsed_then_return_empty_or_a_valid_list(text: str, expected_result: str):
    ...
    # then
    assert expected_result == actual_result, "Didn't find the expected letters in the text"
...
def test_given_a_text_when_counted_then_return_NaN_or_count_of_letters(text: str, expected_result: float):
    # given, when
    actual_result = count_repeated_letters(text)

    # then
    assert expected_result is actual_result, "Didn't find the expected number of letters in the text"
    f"Expected: {expected_result}, Actual: {actual_result}"
